				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						$('#' + settings.queueID).find('.uploadify-queue-item').each(function() {
							...
							$(this).find('.data').removeClass('data').html('<p style="color:#FF0000">ȡ���ϴ�!</p>');
						...
						for (var n = 0; n < args.length; n++) {
							...
							$('#' + args[n]).find('.data').removeClass('data').html('<p style="color:#FF0000">ȡ���ϴ�!</p>');
					...
					$item.find('.data').removeClass('data').html('<p style="color:#FF0000">ȡ���ϴ�!</p>');
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('�ļ���Ϊ: "' + file.name + '" �Ѿ������ڶ�����.\nȷ��Ҫ�����滻?');
					if (!replaceQueueItem) {
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('�ļ��� "' + file.name + '"�Ѿ��ϴ�.\nȷ��Ҫ�滻?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				$('#' + file.id).find('.data').html('<p style="color:#00FF00">上传成功!</p>');
			}
